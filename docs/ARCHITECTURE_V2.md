...
| Multi-tenant agent service | Loops over every active user's config each poll cycle; submits proofs against each user's own instance; platform-owned LLM key (Gemini only today; a provider allow-list with OpenAI Chat Completions/Mistral is the tracked direction — Claude excluded — see docs/CURRENT_REALITY.md). | 3 |
