...
export default function Mandate() {
  ...
  async function deploy() {
    ...
    try {
      ...
      try { await saveInstanceMapping({email:onboarding.email ?? onboarding.authMethod ?? "social",walletAddress:account.address,instanceAddress:treasury}); } catch {}
  ...
  return <Layout><main className="mx-auto max-w-5xl px-6 py-12">
    ...
    <p className="mt-3 max-w-3xl text-sm leading-relaxed text-ledger-400">Start with a profile or customize the safeguards yourself. The AI can recommend actions, but it cannot change these limits.</p>
    <div className="mt-5"><SecurityBoundaryNotice /></div>
    <section className="mt-7 grid gap-3 sm:grid-cols-3">{Object.entries(PRESETS).map(([name,preset])=><button key={name} type="button" onClick={()=>setDraft({...preset,enabledStrategies:{...preset.enabledStrategies}})} className="rounded-lg border border-ledger-700 bg-ledger-900 p-4 text-left hover:border-copper-500"><p className="font-semibold text-ledger-100">{name}</p><p className="mt-1 text-xs text-ledger-400">{name==="Conservative"?"Smaller actions and tighter exposure.":name==="Balanced"?"Recommended default for the controlled market.":"Larger limits and wider operating range."}</p></button>)}</section>
    ...
    <section className="mt-8 rounded-lg border border-copper-700 bg-ledger-900 p-5"><h2 className="text-lg text-ledger-100">Deployment review</h2><p className="mt-2 text-sm text-ledger-400">Owner: <span className="font-data break-all text-ledger-200">{onboarding?.walletAddress ?? "Not connected"}</span></p><p className="mt-1 text-sm text-ledger-400">New treasuries start paused. Fair Witness tops up only onboarding gas on CC3; this user-controlled wallet still signs every owner transaction.</p>{errors.length > 0 && <ul className="mt-4 list-disc pl-5 text-sm text-alert-400">{errors.map(e=><li key={e}>{e}</li>)}</ul>}{error && <p className="mt-4 rounded border border-alert-500/30 bg-alert-500/5 p-3 text-sm text-alert-400">{error}</p>}<button type="button" disabled={busy || errors.length>0 || !onboarding} onClick={()=>void deploy()} className="mt-5 rounded bg-copper-500 px-5 py-3 font-semibold text-ledger-950 disabled:opacity-50">{busy ? "Deploying…" : "Deploy my Fair Witness treasury"}</button></section>
  </main></Layout>;
}
